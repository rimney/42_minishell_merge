...
{
    *in = *in;
    if(exec->command[*index + 1] && ft_is_another_flag(exec, *index + 1) == REDIROUT)
        *fd = open(exec->command[*index + 2], O_CREAT | O_RDWR | O_TRUNC, 0644);
    if(exec->command[*index + 1] && ft_is_another_flag(exec, *index + 1) == APPEND)
        *fd = open(exec->command[*index + 2], O_CREAT | O_RDWR | O_APPEND, 0644);
    if(exec->command[*index + 1] && ft_is_another_flag(exec, *index + 1) == REDIRIN)
        *in = open(exec->command[*index + 2], O_RDONLY);
    if(exec->command[*index + 1] && ft_is_another_flag(exec, *index + 1) == HEREDOC)
