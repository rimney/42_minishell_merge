...
/*   Updated: 2022/07/27 10:14:42 by rimney           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execute_command(t_exec *exec, int index)
{
	...
	command_parser = ft_split_special(exec->command[index], 1);
	...
	{
		if (execve(ft_exec_command(exec->envp, command_parser[0]),
				command_parser, exec->envp) == -1)
		{
